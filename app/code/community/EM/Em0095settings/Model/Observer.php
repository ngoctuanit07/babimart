<?php
class EM_Em0095settings_Model_Observer {
	
	public function beforeGenerateBlocks(Varien_Event_Observer $observer) {
		if((Mage::getSingleton('core/design_package')->getPackageName() == 'default')&& (Mage::getDesign()->getTheme('frontend') == 'em0095')){	
			# Disable default magento navigation
			if (Mage::helper('em0095settings')->getGeneral_DisableDefaultNav()&&(Mage::getConfig()->getModuleConfig('EM_Megamenupro')->is('active', 'true'))) {
			...
			if (Mage::helper('em0095settings')->getGeneral_DisableFrontendVariation()) {
			...
			if (Mage::helper('em0095settings')->getGeneral_DisableFooterLinks()) {
	...
	public function beforeCatalogProductCollectionLoad(Varien_Event_Observer $observer) {
		if((Mage::getSingleton('core/design_package')->getPackageName() == 'default')&& (Mage::getDesign()->getTheme('frontend') == 'em0095')){		
			$alt_img = Mage::helper('em0095settings')->getProductsGrid_AltImg();
			if ($alt_img == 'image')
