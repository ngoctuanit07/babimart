...
class EM_BoughtOfRecentViewedProducts_Block_List extends Mage_Catalog_Block_Product_Abstract
{
    ...
    protected function _construct()
    {
        ...
        $cacheTags = array(Mage_Catalog_Model_Product::CACHE_TAG,self::CACHE_TAG,self::CACHE_TAG.'_'.Mage::getSingleton('log/visitor')->getId());
        if($this->isShowLabel()){
            $cacheTags[] = EM_Productlabels_Model_Productlabels::CACHE_TAG;
        }
		if($this->getData('new_category'))
			$cacheTags[] = Mage_Catalog_Model_Category::CACHE_TAG;
		$this->addData(array(
	...
	public function addCacheTag($cacheTag){
		if($cacheTags = $this->getData('cache_tags')){
			$cacheTags[] = $cacheTag;
		} else {
	...
	public function isShowLabel(){
        return $this->getData('show_label') && Mage::helper('core')->isModuleEnabled('EM_Productlabels');
	}
